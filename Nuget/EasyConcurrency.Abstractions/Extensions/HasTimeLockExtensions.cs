...
public static class HasTimeLockExtensions
{
    ...
    /// otherwise false.
    /// </returns>
    public static bool IsLocked(this IHasTimeLock entity)
    {
        return entity.LockedUntil.IsLocked();
    }

    /// <summary>
    /// Checks whether entity using this TimeLock is not locked.
    /// </summary>
    /// <param name="entity"><see cref="TimeLock"/> instance to check</param>
    /// <param name="now">Specifies the current time to be used when comparing if the entity is locked or not.</param>
    /// <returns>Returns true if entity is not locked and therefore free to be claimed,
    /// otherwise false.
    /// </returns>
    public static bool IsLocked(this IHasTimeLock entity, DateTimeOffset now)
    {
        return entity.LockedUntil.IsLocked(now);
    }
